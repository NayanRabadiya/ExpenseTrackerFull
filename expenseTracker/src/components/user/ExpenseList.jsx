// Expense table with date/amount/category filters, paging, and inline
// edit + delete. Dates move between dd/mm/yyyy (API) and yyyy-mm-dd (inputs),
// which the helpers near the top of this component handle.
...
// Filterable, editable list of the logged-in user's expenses.
export const ExpenseList = () => {
  ...
  // Parse a Date out of either dd/mm/yyyy or yyyy-mm-dd; null if unparseable.
  const toDate = (date) => {
  ...
  // Format for the API ("" if the date can't be parsed).
  ...
  // Format for a native <input type="date"> ("" if unparseable).
  ...
  // Fill the category filter from the list the dashboard cached.
  const getCategories = async () => {
    const categoriesData = localStorage.getItem("categories");
    setCategories(categoriesData ? JSON.parse(categoriesData) : []);
  }

  // Show the cached expenses immediately, then refresh from the API.
  // A user with no expenses gets a 404, which clears the list.
  ...
  // Re-apply every filter, sort newest first, and reset paging if it overflows.
  useEffect(() => {
  ...
  // Confirm, then remove the row optimistically and call the API.
  // If the request fails, the backup is restored.
  ...
  // PUT the edited row and merge the API's response back into the list.
