// Budget page: set a per-category budget, and edit or delete existing ones.
// The API allows only one budget per user+category.
...
// Table of the user's budgets plus a form for adding more.
export const BudgetForm = () => {
  ...
  // POST one budget per queued category, then refresh the cached totals.
  const handleBudgetSubmit = async (data) => {
    ...
    const newBudgets = pendingBudgetCategories.map(({ categoryId, categoryName }) => ({
      ...
      categoryName,
      userId: userId,
      // Form inputs give strings; coerce here so every total downstream adds
      // numerically instead of concatenating.
      amount: Number(data[`amount_${categoryId}`]),
    }));
    ...
    toast.promise(
      (async () => {
        ...
        const addedAmount = savedBudgets.reduce((total, budget) => total + Number(budget.amount), 0);
        const newTotalBudget = Number(totalBudget) + addedAmount;
  ...
  // PUT the edited budget, then recompute and re-cache the total.
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const newTotalBudget = updatedList.reduce((total, budget) => total + Number(budget.amount), 0);
  ...
  // Confirm, delete the budget, then update the cached list and total.
