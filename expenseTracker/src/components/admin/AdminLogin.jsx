...
// Admin login page (/login/admin). Only accounts holding the admin role get in.
export const AdminLogin = () => {
  ...
  // Post the credentials, then save the login marker and open the dashboard.
  const onSubmit = async (data) => {
    try {


      const response = await toast.promise(axios.post("/login/admin", data), {
        pending: "Logging in...",
      ...
      if (response.status == 200 && String(response.data?.role?.name || "").toLowerCase() == "admin") {
        localStorage.clear();
        ...
        // Logged in, but the account isn't an admin — say so instead of
        // silently staying on this page.
